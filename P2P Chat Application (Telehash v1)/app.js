...
if (!process.argv[2]) {
    console.log("Usage: node app.js nickName [chatRoom]\n");
    process.exit();
}

nickName = process.argv[2];
if (process.argv[3]) {
    chatRoom = process.argv[3];
}
    ...
    if (!connector) {
        ...
        telehash.listen(name, function (MSG) {
            ...
            if (!chatCache[msg_sig]) {
                if (MSG.message.x) {
                    if (MSG.message.x == 'join') {
                        console.log("[JOINED] " + MSG.message.nick);
                    }
                    if (MSG.message.x == 'leave') {
                        console.log("[LEFT THE CHAT] <" + MSG.message.nick + ">");
                    }
                } else {
                    if (MSG.message.txt) {
                        console.log("<" + MSG.message.nick + ">: " + MSG.message.txt);
                    }
                }
